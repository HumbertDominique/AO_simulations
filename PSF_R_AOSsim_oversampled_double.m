...
for k=1:nIteration
    ...
    if SAVELIGHTFIELD
        lightfieldHistory(:,:,indexInBatch) = wfs.camera.frame*255;
    end
    if SAVEDM
    ...
    if mod(k, batchItSize) == 0 || k == nIteration
        ...
        if SAVELIGHTFIELD
            ...
            h5create(outputDir+"\"+fileID_lightfield+string(batchIndex)+".h5", '/wf_lightfield', sz, 'ChunkSize', [sz(1) sz(2) 1],'DataType', 'double');
        ...
        if SAVEPSF
            sz = size(psfHistory);
            h5create(outputDir+"\"+fileID_ipsf+string(batchIndex)+".h5", '/psf_history', sz, 'ChunkSize', [sz(1) sz(2) 1], 'DataType', 'double');
